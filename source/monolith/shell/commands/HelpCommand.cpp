...
std::map<std::string, std::string> CommandNameToDescription() {
  return {
      ...
      {   "runnb",
       "Runs the executable in non-blocking mode and times execution. Usage: `runnb <program_name>`"},
      {"runnbfor",
       "Runs the executables in non-blocking mode several times, substituting the"
       " variable name, and times execution. Usage: `runnbfor <name>=<start>:<end>[:<step>] "
       "<program_name> [...args]`"                                                                  }
  };
}
